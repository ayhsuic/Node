...
const { SocksClient } = require('socks'); // [新增] 引入 socks 库
...
wss.on('connection', (ws) => {
    ws.once('message', (msg) => {
        ...
        // [修改核心点] 将 net.connect 改为 SocksClient 代理连接
        SocksClient.createConnection({
            proxy: {
                host: '127.0.0.1',
                port: 1080,    // 本地 WARP (Wireproxy) 的 SOCKS5 端口
                type: 5        // SOCKS5
            },
            command: 'connect',
            destination: {
            ...
            const tcpSocket = info.socket;
            // 将 VLESS 的载荷写入代理后的 TCP 流
            tcpSocket.write(msg.slice(i));
            duplex.pipe(tcpSocket).pipe(duplex);
            
            tcpSocket.on('error', () => {});
        }).catch(err => {
            console.error("代理连接失败，可能 WARP 未就绪:", err.message);
        });
